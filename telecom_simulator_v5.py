...
# GDPR right-to-erasure: a random subscriber requests all their records deleted.
SUBSCRIBER_ERASURE_DAILY_PROB = 0.005  # daily chance a subscriber triggers erasure
...
-- Index for subscriber erasure (GDPR: mark all rows for an imsi_hash as deleted)
CREATE INDEX IF NOT EXISTS idx_st_imsi_hash
    ON telecom.subscriber_traffic(imsi_hash);
...
def simulate_soft_deletes(
    ...
    """Simulate soft deletes on dimension and fact tables.

    Dimension tables:
      - Operators get acquired/merged → is_deleted = true
      - Locations get redistricted    → is_deleted = true

    Fact tables (GDPR right-to-erasure):
      - A random subscriber (imsi_hash) requests all their records deleted.
        Every row in subscriber_traffic, performance_metrics (via station),
        and station_events that references their station activity is marked
        is_deleted = true with updated_at bumped so the ETL watermark picks
        it up and can propagate the tombstone downstream.
    ...
    with _pg_conn(db) as conn:
        with conn.cursor() as cur:
            ...
            # GDPR erasure: a subscriber requests deletion of all their data.
            # Marks every subscriber_traffic row for that imsi_hash, plus all
            # performance_metrics and station_events rows on their home station
            # (proxy for "data generated by this subscriber's presence").
            if rng.random() < SUBSCRIBER_ERASURE_DAILY_PROB:
                cur.execute("""
                    SELECT imsi_hash, station_id
                    FROM telecom.subscriber_traffic
                    WHERE is_deleted = false
                    ORDER BY RANDOM()
                    LIMIT 1
                """)
                row = cur.fetchone()
                if row:
                    imsi_hash, station_id = row

                    # All traffic records for this subscriber
                    cur.execute("""
                        UPDATE telecom.subscriber_traffic
                        SET is_deleted = true, updated_at = NOW()
                        WHERE imsi_hash = %s AND is_deleted = false
                    """, (imsi_hash,))
                    deleted += cur.rowcount

                    # Performance metrics on their most-used station
                    cur.execute("""
                        UPDATE telecom.performance_metrics
                        SET is_deleted = true, updated_at = NOW()
                        WHERE station_id = %s AND is_deleted = false
                    """, (station_id,))
                    deleted += cur.rowcount

                    # Station events on that same station
                    cur.execute("""
                        UPDATE telecom.station_events
                        SET is_deleted = true, updated_at = NOW()
                        WHERE station_id = %s AND is_deleted = false
